...
export default {
  ...
  effects: {
    *fetch({ payload }, { call, put }) {
      ...
      const result = yield call(queryMenuResource, payload);
    ...
    *fetchById({ payload, callback }, { call, put }) {
      const response = yield call(getMenuResource, payload);
      callback(response);
    },
    *deleteByIds({ payload, callback }, { call, put }) {
      const response = yield call(deleteMenuResource, payload);
      if (response) {
        callback(response);
      }
    },
    *submitForm({ payload, callback }, { call, put }) {
      const response = !payload.id
        ? yield call(addMenuResource, payload)
        : yield call(updateMenuResource, payload);
      if (response) {
        callback(response);
      }
    },
